...
#define GH_CLASS_NAME "goodix_spo2"
#define GH_INPUT_NAME "gh-keys"

#define GH_LINUX_VERSION "V1.01.05"
...
static struct gh_device *s_gh_dev = NULL;

#define WAKE_UP_CMD  (0xC3)
#define SLEEP_CMD    (0xC4)
typedef struct {
    unsigned short addr;
    unsigned short val;
} gh_cfg_pair_t;

gh_cfg_pair_t gh_cfg_pairs[] = {
    {0x0036, 0x0000},  // chip ready, 0xAA55
    {0x0508, 0x0000},  // chip int reg, 0x4000
    {0x0034, 0x0000},  // chip id, 0x0011
    {0x0030, 0x0000},  // product id low
    {0x0032, 0x0000},  // product id high
    {0x01EC, 0x0000},  // chip slot test
};

#if (GH_SUPPORT_BUS_SPI == GH_SUPPORT_BUS)
static void gh_class_sendcmd(unsigned char cmd)
{
    unsigned char read_buf[1] = { 0 };
    unsigned char write_buf[1] = { 0 };
    write_buf[0] = cmd;
    gh_spi_setup_conf(s_gh_dev, 6);
    gh_spi_transfer_raw(s_gh_dev, write_buf, read_buf, 1);
    udelay(500);

    return;
}
static void gh_class_write(unsigned short addr, unsigned short val)
{
    ...
    write_buf[3] = 0;
    write_buf[4] = 2;
    write_buf[5] = (val >> 8) & 0xFF;
    write_buf[6] = val & 0xFF;

    gh_spi_setup_conf(s_gh_dev, 6);
    gh_spi_transfer_raw(s_gh_dev, write_buf, read_buf, 7);

    return;
}

static void gh_class_read(unsigned short addr, unsigned short *val)
{
    unsigned char read_buf[7] = { 0 };
    unsigned char write_buf[7] = { 0 };
    write_buf[0] = 0xF0;
    ...
    gh_spi_setup_conf(s_gh_dev, 6);
    gh_spi_transfer_raw(s_gh_dev, write_buf, read_buf, 3);

    memset(read_buf, 0 ,7);
    memset(write_buf, 0 ,7);
    write_buf[0] = 0xF1;
    gh_spi_transfer_raw(s_gh_dev, write_buf, read_buf, 3);

    *val = read_buf[1];
    *val = (*val << 8) + read_buf[2];
...
static void gh_class_sendcmd(unsigned char cmd)
{
    ...
    ret = i2c_transfer(s_gh_dev->client->adapter, &msgs, 1);
    if (0 > ret)
    {
...
static void gh_class_read(unsigned short addr, unsigned short *val)
{
    ...
    *val = read_buf[0];
    *val = (*val << 8) + read_buf[1];

    return;
}
#endif

static ssize_t gh_reg_show(struct class *class,
                             struct class_attribute *attr, char *buf)
{
    ...
    mutex_lock(&s_gh_dev->buf_lock);

    gh_class_sendcmd(WAKE_UP_CMD);

    for (i = 0; i < sizeof(gh_cfg_pairs) / sizeof(gh_cfg_pairs[0]); i++)
    {
        val = 0;
        gh_class_read(gh_cfg_pairs[i].addr, &val);
        sprintf(temp_buf , "reg:0x%04X=0x%04X\n", gh_cfg_pairs[i].addr, val);
        strcat(buf, temp_buf);
    }
    gh_debug(DEBUG_LOG, "%s\n", buf);

    gh_class_sendcmd(SLEEP_CMD);

    mutex_unlock(&s_gh_dev->buf_lock);

    return strlen(buf);
}

static ssize_t gh_reg_store(struct class *class, struct class_attribute *attr,
                              const char *buf, size_t count)
{
    unsigned short databuf[2] = {0, 0};

    mutex_lock(&s_gh_dev->buf_lock);

    gh_class_sendcmd(WAKE_UP_CMD);

    if (2 == sscanf(buf, "%hx %hx", &databuf[0], &databuf[1])) {
        gh_class_write(databuf[0], databuf[1]);
        gh_debug(DEBUG_LOG, "addr:0x%04X, val:0x%04X\n", databuf[0], databuf[1]);
    }else {
        gh_debug(DEBUG_LOG, "input is invaild");
    }
...
static struct class_attribute gh_class_attr =
              __ATTR(reg, S_IRUGO | S_IWUSR, gh_reg_show, gh_reg_store);
...
#endif
{
	...
	/* create class file */
	status = class_create_file(gh_dev->class, &gh_class_attr);
	if (status) {
		gh_debug(ERR_LOG, "%s, Failed to create class file.\n", __func__);
		status = -ENODEV;
		goto err_sysfs;
	} else {
		gh_debug(INFO_LOG, "%s, Success create class file.\n", __func__);
	}
...
err_cdev:
	class_remove_file(gh_dev->class, &gh_class_attr);
