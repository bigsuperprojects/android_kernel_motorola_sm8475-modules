			 ...
			 macro_pkg_img_t image_no)
{
	...
	if (*(uint32_t *)&fw->data[0] ==
		...
		//write selected image
		rc = qm357xx_rom_unpack_fw_macro_pkg(fw, &all_fws, image_no);
		if (rc) {
...
#ifdef FWU_FB_ROM_CODE
	struct fw_pkg_hdr_t *fw_pkg_hdr;
#endif

	rc = qm357xx_rom_fw_macro_pkg_get_fw_idx(fw, image_no, &fw_size,
						 &fw_data);
	if (rc) {
...
#ifdef FWU_FB_ROM_CODE
	fw_pkg_hdr = (struct fw_pkg_hdr_t *)fw_data;
	if (fw_pkg_hdr->enc_mode != 0) {
		LOG_ERR("%s: FW PACKAGE encrypted (enc_mode = %d)\n", __func__,
			fw_pkg_hdr->enc_mode);
		return -EROFS;
	}
#endif
